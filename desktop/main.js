...
// --- Chromium switches ------------------------------------------------
//
// autoplay-policy=no-user-gesture-required — без этого в десктопе не
// работает обработка микрофона, включая AI-шумодав.
//
// Механика: цепочка звонка асинхронная (клик -> await getUserMedia ->
// createMicPipeline -> new AudioContext). К моменту создания контекста
// Chromium уже не считает тот клик «живым» жестом пользователя, поэтому
// ctx.resume() не отрабатывает и контекст остаётся в состоянии
// 'suspended'. MediaStreamDestination при этом отдаёт немой трек, и
// watchdog в useCall/useGroupCall честно откатывается на сырой микрофон —
// звонок идёт, но без фильтров и без шумодава.
//
// В браузере то же самое работает, потому что там страница живёт дольше
// и обычно уже имеет sticky activation. Отсюда и симптом «в вебе шумодав
// есть, в десктопе гул вентилятора».
//
// Тест микрофона в настройках при этом работал всегда: там AudioContext
// создаётся непосредственно в обработчике клика, активация ещё жива.
// Именно это расхождение и сбивало с толку при диагностике.
//
// Ослабление политики автовоспроизведения здесь безопасно: приложение
// само по себе является аудио-клиентом, чужие страницы в нём не
// открываются (внешние ссылки уходят в системный браузер).
